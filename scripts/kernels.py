...
#-----------------------------------------------------------
# Functions for zero standard deviation kernels O(h^4)
# using procedure from Monaghan (1992)
#-----------------------------------------------------------
def superkernel(wref, R):
    f, name = wref(R)
    c1D, c2D, c3D = getnorm(f, R)
    mom2 = integrate(c1D * q**2 * f, (q, 0, R))
    mom4 = integrate(c1D * q**4 * f, (q, 0, R))
    A = mom2 / mom4
    norm = integrate(f * (1 - A * q**2), (q, 0, R))
    B = 1 / norm
    g = piecewise_fold(B * f * (1 - A * q**2))
    name = f"Superkernel O(h^4) {name} (A={A}, B={B})"
    return g, name

def superkernel3D(wref, R):
    f, name = wref(R)
    mom2_3d = integrate(4*pi*q**4 * f, (q, 0, R))
    mom4_3d = integrate(4*pi*q**6 * f, (q, 0, R))
    A = mom2_3d / mom4_3d
    norm_3d = integrate(4*pi*q**2 * f * (1 - A * q**2), (q, 0, R))
    B = sympify(1) / norm_3d
    g = piecewise_fold(B * f * (1 - A * q**2))
    name = f"Zero-stddev superkernel3D O(h^4) {name} (A={A}, B={B})"
    return g, name
...
def printkernel_phantom(w,R,name):
    ...
    print (" real, parameter, public  :: cnormk_tilde = cnormk")
    print (" real, parameter, public  :: wab0_tilde = wab0, gradh0_tilde = gradh0")
    ...
    print ("pure subroutine get_kernel_tilde(q2,q,wkern,grkern)")
    print (" real, intent(in)  :: q2,q")
    print (" real, intent(out) :: wkern,grkern")
    print ("")
    print (" call get_kernel(q2,q,wkern,grkern)")
    print ("")
    print ("end subroutine get_kernel_tilde\n")
...
def plotkernel(w,R,name):
    import matplotlib.pyplot as plt
    import numpy as np
    from sympy import lambdify
    q_vals = np.linspace(0, float(R), 100)
    w_func = lambdify(q, w, 'numpy')
    w_vals = w_func(q_vals)
    plt.figure(figsize=(10, 6))
    plt.plot(q_vals, w_vals, 'b-', linewidth=2, label=name)
    plt.title(f'Kernel Function: {name}')
    plt.xlabel('q')
    plt.ylabel('w(q)')
