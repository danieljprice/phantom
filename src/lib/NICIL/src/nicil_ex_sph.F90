...
program nicil_ex_sph
 ...
 write(iprintdat,"('#',9(1x,'[',i2.2,1x,a11,']',2x))") &
       1,'density',    &
       2,'B_z',        &
       3,'J_y',        &
       4,'dB/dt_ni,z', &
...
!$omp shared(n_R,n_electronT,vion) &                                                                 !for NICIL
...
!$omp do schedule(runtime)
   do i = ixmin,ixmax  ! should be i = 1,npart, but for speed, only update the middle values
     ...
     call nicil_get_vion(eta_ambii,vxi,vyi,vzi,Bxi,Byi,Bzi,jcurrent(1:3,i),vion(1:3,i),ierr)
     if (ierr/=0) then
       call nicil_translate_error(ierr)
       call fatal(ierr,rho(i),temperature)
     end if
...
!$omp end parallel
 ...
 do i = ixmin,ixmax
   write(iprintdat,'(9(1pe18.10,1x))') &
         rho(i)*unit_density,B(3,i)*unit_Bfield,jcurrent(2,i)*unit_Bfield/udist**3, &
         dBdt(3,i)*unit_Bfield/utime,eta(:,i)*unit_eta,dudt(i)*unit_energy/utime,vion(1,i)*unit_velocity
 end do
