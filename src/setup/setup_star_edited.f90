...
subroutine setpart(id,npart,npartoftype,xyzh,massoftype,vxyzu,polyk,gamma,hfact,time,fileprefix)
 ...
 use units, only:udist!i added this line
 ...
 real, allocatable                :: r(:),den(:),pres(:),temp(:),en(:),mtab(:),Xfrac(:),Yfrac(:),composition(:,:),comp(:) !i added last two
 real                             :: eni,tempi,p_on_rhogas,xi,yi,zi,ri,spsoundi,densi,hi
 integer                          :: columns_compo !I added these two lines
 ...
 case(ikepler)
    call read_kepler_file(trim(input_profile),ng_max,npts,r,den,pres,temp,en,Mstar,composition,columns_compo,ierr)
    if (ierr==1) call fatal('setup',trim(input_profile)//' does not exist')
    if (ierr==2) call fatal('setup','insufficient data points read from file')
    if (ierr==3) call fatal('setup','too many data points; increase ng')
    rmin  = r(1)
 ...
 !
 ! open(11,file='output_00000')
 ! write(11,"('#',21(1x,'[',1x,a28,']',2x))") &
 !       'radius',&
 !       'nt1',&
 !       'h1',&
 !       'he4',&
 ...
 !       'fe56',&
 !       "'fe'"
 ! if (composition_exists .eqv. .true.) then
 !   print*, 'Saving the stellar composition',nstar,'nstar'
 ...
 !     end do
 !       write(11,*) ri*udist,compositioni
 !   end do
 ! end if
 ! close(11)
 !
 ...
 if (maxvxyzu <= 4) then
    ...
    print*, polyk*Rstar/Mstar, 'polyk*Rstar/Mstar',polyk,'polyk',Rstar,'Rstar',Mstar,'Mstar'
 endif
