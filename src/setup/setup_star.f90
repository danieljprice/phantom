...
subroutine setpart(id,npart,npartoftype,xyzh,massoftype,vxyzu,polyk,gamma,hfact,time,fileprefix)
 ...
 if (setexists) then
    call read_setupfile(setupfile,gamma,need_iso,ierr)
    if (ierr /= 0) then
 ...
 elseif (id==master) then
    ...
    call setup_interactive(gamma,iexist,id,master,ierr)
...
!+
!-----------------------------------------------------------------------
subroutine setup_interactive(gamma,iexist,id,master,ierr)
 ...
 ! star
 !call set_star_interactive(id,master,star,need_iso,use_var_comp,ieos)
...
subroutine write_setupfile(filename,gamma,polyk)
 ...
 use eos,           only:ieos
 use eos_gasradrec, only:irecomb
 use setstar,       only:write_options_stars,need_polyk
 ...
 call write_options_stars(star,relax_star_in_setup,ieos,iunit)
...
!+
!-----------------------------------------------------------------------
subroutine read_setupfile(filename,gamma,need_iso,ierr)
 use infile_utils,  only:open_db_from_file,inopts,close_db,read_inopt
 use io,            only:error
 use units,         only:select_unit
 use relaxstar,     only:read_options_relax
 use eos,           only:X_in,Z_in,gmw,ieos
 use eos_gasradrec, only:irecomb
 ...
 ! star options
 call read_options_stars(star,need_iso,ieos,relax_star_in_setup,db,nerr)
