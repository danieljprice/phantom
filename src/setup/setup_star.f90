...
subroutine setpart(id,npart,npartoftype,xyzh,massoftype,vxyzu,polyk,gamma,hfact,time,fileprefix)
 ...
 !
!  do i=1,npts
!     call calc_temp_and_ene(den(i)*unit_density,pres(i)*unit_pressure,en(i),temp(i),ierr)
!  enddo
!  call write_softened_profile(outputfilename,mtab*umass,pres*unit_pressure,temp,r*udist,den*unit_density,en)
!  stop
...
subroutine read_setupfile(filename,gamma,polyk,ierr)
 ...
 if (iprofile==imesa) then
    if (isoftcore > 0) call read_inopt(outputfilename,'outputfilename',db,errcount=nerr)
    ...
    case(2,3) ! fixed entropy softened core
