...
!  use io,   only:warning,fatal
 ...
 logical               :: iexist
 integer               :: io, lines
 real                  :: period_ratio,criterion,q_comp,a_comp,e_comp,m_comp,rel_posang_ascnode,rel_arg_peri,rel_incl
 ...
 ! masses
 Rochelobe1 = Rochelobe_estimate(msecondary,mprimary,semimajoraxis)
 Rochelobe2 = Rochelobe_estimate(mprimary,msecondary,semimajoraxis)
 reducedmass = mprimary*msecondary/mtot
 angmbin = reducedmass*sqrt(mtot*semimajoraxis*(1. - eccentricity**2))
 ...
 if (do_verbose) then
    ...
    print "(8(2x,a,g12.3,/),2x,a,g12.3)", &
        ...
        'secondary mass   :',msecondary, &
        'mass ratio m2/m1 :',msecondary/mprimary, &
        'reduced mass     :',reducedmass, &
        'semi-major axis  :',semimajoraxis, &
        'period           :',period, &
        'eccentricity     :',eccentricity, &
 ...
 if (do_verbose .and. present(subst)) then
    print "(2x,a,g12.3)", &
         'star to be subst :',subst
 endif
...
!
 ...
 dx = 0.
 dv = 0.
 if ((present(posang_ascnode) .and. present(arg_peri) .and. present(incl)) .or. present(subst)) then
    ! Campbell elements
    a = semimajoraxis
    ecc = eccentricity
    ...
    term1 = cos(E)-eccentricity
    term2 = sqrt(1.-(eccentricity*eccentricity))*sin(E)
    E_dot = sqrt((mprimary + msecondary)/(a**3))/(1.-eccentricity*cos(E))

    if (do_verbose) then
       print "(4(2x,a,g12.4,/),2x,a,g12.4)", &
             'Eccentric anomaly:',E, &
             'E_dot            :',E_dot, &
             'inclination     (i, deg):',rel_incl, &
             'angle asc. node (O, deg):',rel_posang_ascnode, &
             'arg. pericentre (w, deg):',rel_arg_peri
       if (present(f)) print "(2x,a,g12.4)", &
             'true anomaly    (f, deg):',f
    endif
 ...
 ! positions of each star so centre of mass is at zero
 x1 = -dx*msecondary/mtot
 x2 =  dx*mprimary/mtot

 ! velocities
 v1 = -dv*msecondary/mtot !(/0.,-m2/mtot*vmag,0./)
 v2 =  dv*mprimary/mtot !(/0.,m1/mtot*vmag,0./)
...
!
 ...
 xyzmh_ptmass(1:3,i1) = x1
 xyzmh_ptmass(1:3,i2) = x2
 xyzmh_ptmass(4,i1) = mprimary
 xyzmh_ptmass(4,i2) = msecondary
 xyzmh_ptmass(5,i1) = accretion_radius1
 xyzmh_ptmass(5,i2) = accretion_radius2
 xyzmh_ptmass(6,i1) = 0.0
 xyzmh_ptmass(6,i2) = 0.0
...
! rotate if inclination is non-zero
!
 if (present(incl) .and. .not.(present(arg_peri) .and. present(posang_ascnode)) .and. .not. present(subst)) then
    xangle = rel_incl*pi/180.
    cosi = cos(xangle)
    sini = sin(xangle)
    do i=i1,i2
       call rotate(xyzmh_ptmass(1:3,i),cosi,sini)
 ...
 if (present(subst)) then
    if (subst>0) then ! Rotate the substituting binary with orientational parameters referring to the substituted star's orbital plane
       omega     = rel_arg_peri*pi/180.
       !big_omega = rel_posang_ascnode*pi/180.! + 0.5*pi
       inc       = rel_incl*pi/180.
