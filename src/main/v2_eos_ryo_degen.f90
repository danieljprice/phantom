...
! -------------------------------------------------------------------------------
subroutine init_eos_ryo(x, z, ierr)
  ...
  write(*,*) 'Init degenerate EOS with X=', x, ' Z=', z
...
! Get good initial guess if guess is not provided
 ...
 subroutine get_pT_from_de_ryo(d,e,mue,mui,p,T,eta)
  ...
  real(8):: theta,eta1,pele,nele

  if(present(eta))eta1 = min(eta,etalim)
  call etaT_from_de(d,e,mue,mui,eta1,T)

  theta = fac_theta*T
  call get_nele(eta1,theta,nele)
  if(T>0d0.and.abs(nele*mue*amu/d-1d0)<1d-3)then
 ...
 subroutine get_p_from_dT(d,T,mue,mui,p,eta,dpdt,dpdd)
  ...
  if(present(dpdt).and.present(dpdd))then
   call get_pele(eta1,theta,pele,dpeledt=dpeledt,dpeledne=dpeledd)
  elseif(present(dpdt))then
   call get_pele(eta1,theta,pele,dpeledt=dpeledt)
  elseif(present(dpdd))then
   call get_pele(eta1,theta,pele,dpeledne=dpeledd)
  else
   call get_pele(eta1,theta,pele)
  end if
...
!   use fermi_dirac_approx
  ...
  real(8):: eta1,theta1,deeledt_eta,deeledeta_t,dnedt_eta,dnedeta_t

! Limit eta value to <etalim and use cold formula instead
!!$  eta1 = min(eta,etalim)
!!$  if(abs(eta1)<epsilon(1d0))then
!!$   theta1 = theta
!!$  else
...
!!$  end if
  ...
  if(present(dnelede))then
   call dfermi(0.5d0,eta1,theta1,f12,fdeta=df12de)
   call dfermi(1.5d0,eta1,theta1,f32,fdeta=df32de)
  else
   call dfermi(1.5d0,eta1,theta1,f12)
   call dfermi(2.5d0,eta1,theta1,f32)
  end if
...
! Limit eta value to <etalim and use cold formula instead
 ...
 function Efermi(ne) result(E)
  real(8),intent(in):: ne
  real(8):: E
  E = hbar*hbar/(2d0*m_e)*(3d0*pi*pi*ne)**(2d0/3d0)
 end function Efermi
...
! Avoid tedious calculation for extremes
 ...
 function Edeg(ne) result(E)
  ...
  x = (fac_pf*ne)**(1d0/3d0)
  sqrtx2p1 = sqrt(1d0+x*x)
  E = pi*(m_e*clight)**4*clight/(3d0*hplanck*hplanck*hplanck)* &
  (8d0*x**3*(sqrtx2p1-1d0)-(x*(2d0*x**2-3d0)*sqrtx2p1+3d0*log(x+sqrtx2p1)))
 end function Edeg
...
!!$   weight = 1d0/(1d0+exp(thsmooth*(thtail-thbulk)))
!!$   dthetaratio = ((1d0-weight)*dthetatail+weight*dthetabulk)/thratio
