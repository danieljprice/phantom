...
!Initialize the mesa_microphysics module
 mesa_eos_z1=0.9987d0   ! lowest Z
 mesa_eos_h1=0.d0   ! lowest H
 mesa_eos_dz=0.0001d0 ! delta Z
 mesa_eos_dh=0.0001d0  ! delta H in EOS tables
!Initialize the mesa_microphysics module
!  mesa_eos_z1=0.975d0   ! lowest Z
!  mesa_eos_h1=0.d0   ! lowest H
!  mesa_eos_dz=0.005d0 ! delta Z
!  mesa_eos_dh=0.0001d0  ! delta H in EOS tables
...
!----------------------------------------------------------------
 ...
 integer                    :: ierr
!  real :: pres_test, logP_wd, logRho_wd !ALi added
!  logical, save :: printed_once = .false.   ! Ali add this

!  logP_wd = 2.1091448051226969E+001
!  logRho_wd =  5.1482766967364588E+000
! ! close to WD surface
!  logP_wd = 1.0541138605780109E+001 
!  logRho_wd = -3.1990621134072494E+000 
!  logP_wd = 1.8562587084790497E-001  
!  logRho_wd =1.4887269853546206E+001 

! close to WD center
!  logP_wd = 2.3034126447404347E+001
!  logRho_wd =  6.4041484326426739E+000 
 ...
 !Ali added following line for debugging purposes, to check that the bisection method is working correctly. Should be removed after testing.
!  write(*,*) "forward check: values found for eint=",eint," temp=",temp
!  call getvalue_mesa(rho,eint,2,pres_test,ierr)
!  write(*,*) "backward check: values found with getvalue_mesa(rho,eint,2,pres,ierr) for pres=",pres_test
 ! Only print once for debugging
!  if (.not. printed_once) then
!     write(*,*) "forward check: values found for eint=",eint," temp=",temp
!     call getvalue_mesa(rho,eint,2,pres_test,ierr)
!     write(*,*) "backward check: pres=",pres_test," target pres=",pres
!     printed_once = .true.
!  end if
!  write(*,*) "DEBUG: Ali"
!  write(*,*) "DEBUG, in cgs: logrho=",log10(rho)," logpres=",log10(pres)
 ! pres and rho are already in cgs, idk why
!  if (abs(log10(rho) - logRho_wd)/abs(logRho_wd) < 1e-2 .and. &
!     abs(log10(pres) - logP_wd)/abs(logP_wd) < 1e-2) then
!     write(*,*) "DEBUG MATCH FOUND"
!     write(*,*) "DEBUG, in cgs: logrho=",log10(rho)," logpres=",log10(pres)
!     write(*,*) "forward check: values found for eint=",eint," temp=",temp
!     call getvalue_mesa(rho,eint,2,pres_test,ierr)
!     write(*,*) "backward check: values found with getvalue_mesa(rho,eint,2,pres,ierr) for logpres=",log10(pres_test)
!  end if
