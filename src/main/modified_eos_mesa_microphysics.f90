...
!For mesa_microphysics
 integer            :: mesa_eos_nz=2, mesa_eos_nh=2   !Number of Z values in EOS tables, number of hydrogen values in EOS tables
 real               :: mesa_eos_z1=0.9987d0, mesa_eos_h1=0.d0, mesa_eos_dz=0.0001d0, mesa_eos_dh=0.0001d0 !lowest Z, lowest H, delta Z, delta H in EOS tables
!  integer            :: mesa_eos_nz=2, mesa_eos_nh=2   !Number of Z values in EOS tables, number of hydrogen values in EOS tables
!  real               :: mesa_eos_z1=0.975d0, mesa_eos_h1=0.d0, mesa_eos_dz=0.005d0, mesa_eos_dh=0.0001d0 !lowest Z, lowest H, delta Z, delta H in EOS tables
...
!  fnum = 121
!  fnum = 501 !Ali: for 500x500 tables
 fnum = 401 !Ali: for 200x200 tables
...
!  fmt1 = '(F4.2)' !
 fmt1 = '(F6.4)' ! Ali changed to read 0.0001 and 0.9750 for x and z

!  fnum = 122
!  fnum = 502 !Ali: for 500x500 tables
 fnum = 402 !Ali: for 200x200 tables
...
subroutine read_eos_mesa(x,z,ierr)
 ...
 real                 :: dz,dx
 integer              :: fnum
!  character(len=300) :: outfilename !Ali Added
!  integer :: outunit !Ali Added

!  fmt1 = '(F4.2)'
 fmt1 = '(F6.4)' ! Ali changed to read 0.0001 and 0.9750 for x and z

!  fnum = 120
!  fnum = 500 !Ali: for 500x500 tables
 fnum = 400 !Ali: for 200x200 tables
...
!----------------------------------------
! Write interpolated EOS table to file: Ali
!----------------------------------------

!  write (zz,fmt1) z
!  write (hh,fmt1) x

!  outfilename = 'interpolatedoutput_DE_z'//trim(zz)//'x'//trim(hh)//'.bindata'

!  outunit = 600
!  open(unit=outunit,file=trim(outfilename),status='replace', &
!      form='unformatted',action='write')

!  ! Write header (same structure as original files)
!  write(outunit) mesa_eos_ne, mesa_eos_nv, mesa_eos_nvar2
!  write(outunit) (mesa_eos_logVs(k),k=1,mesa_eos_nv)
!  write(outunit) (mesa_eos_logEs(l),l=1,mesa_eos_ne)

!  do k=1,mesa_eos_nv
!     do l=1,mesa_eos_ne
!        write(outunit) (mesa_de_data(l,k,m),m=1,mesa_eos_nvar2)
!     enddo
!  enddo
...
!  if (.false.) then !Ali: to enforce monotonicty by using the bilinear thing in the else. 
