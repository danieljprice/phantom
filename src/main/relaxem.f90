...
module relaxem
  !
  ! Routine to shuffle particles towards a reference distribution
  !
  ! :References: None
  !
  ...
  ! :Runtime parameters:
  !
  ! :Dependencies: timestep, part, kernel, bound, kdtree
  !
  implicit none
...
subroutine shift_particles(npart,a_ref,nrelax,relaxlist,ke,maxshift)
  ...
  !$omp parallel do schedule(guided) default(none) &
  !$omp shared(npart,xyzh,vxyzu,fxyzu,ieos,a_ref,maxshift,apr_level) &
