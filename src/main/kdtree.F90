...
subroutine construct_root_node(np,nproot,irootnode,ndim,xmini,xmaxi,ifirstincell,xyzh)
 ...
 !$omp parallel default(none) &
 !$omp shared(np,xyzh) &
 !$omp shared(inodeparts,iphase,xyzh_soa,iphase_soa,nproot,apr_level_soa,apr_weight_soa) &
#ifdef PERIODIC
 ...
 do i=1,np
    isnotdead: if (.not.isdead_or_accreted(xyzh(4,i))) then
       ...
       if (use_apr) then
         ...
         apr_weight_soa(nproot) = apr_weight(i)
       endif
    endif isnotdead
 enddo
...
!
 ...
 if (npnode > 1000 .and. doparallel) then
    ...
    !$omp shared(xyzh_soa,apr_level_soa,apr_weight_soa,i1,iphase_soa) &
    ...
    do i=i1,i1+npnode-1
       ...
       if (maxphase==maxp) then
         if (use_apr) then
           pmassi = massoftypefunc(iamtype(iphase_soa(i)),apr_level_soa(i),apr_weight_soa(i))
         else
...
!--for gravity, we need the centre of the node to be the centre of mass
 ...
 !!$omp shared(npnode,xyzh_soa,x0,i1,apr_level_soa,apr_weight_soa) &
...
!$omp parallel default(none) &
!$omp shared(maxp,maxphase) &
!$omp shared(xyzh, ifirstincell, ncells, apr_level,apr_weight) &
 ...
 levels: do level = 0, globallevel
    ...
    do i=1,np
       ...
       if (use_apr) then
         ...
         apr_weight_soa(npnode) = apr_weight(i)
       endif
    enddo
