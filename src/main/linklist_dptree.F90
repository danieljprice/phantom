...
subroutine update_hmax_remote()
 use dptree, only:climb_tree_hmax,maxlevel

 ! sweep up levels
 call climb_tree_hmax(node,maxlevel)
...
!
 ...
 if (present(f)) then
    ...
    if (present(remote_export)) then
       remote_export = .false.
       call getneigh1(nodeglobal,xpos,xsizei,rcuti,3,listneigh,nneigh,xyzh,xyzcache,ixyzcachesize,&
                      get_j,fgrav_global,remote_export=remote_export)
    endif
#endif
    call getneigh1(node,xpos,xsizei,rcuti,3,listneigh,nneigh,xyzh,xyzcache,ixyzcachesize,get_j,fgrav)
    ...
    if (present(remote_export)) then
       remote_export = .false.
       call getneigh1(nodeglobal,xpos,xsizei,rcuti,3,listneigh,nneigh,xyzh,xyzcache,ixyzcachesize,&
                      get_j,remote_export=remote_export)
    endif
#endif
    call getneigh1(node,xpos,xsizei,rcuti,3,listneigh,nneigh,xyzh,xyzcache,ixyzcachesize,&
                  get_j)
 endif
...
subroutine getneigh_pos(xpos,xsizei,rcuti,ndim,listneigh,nneigh,xyzh,xyzcache,ixyzcachesize,ifirstincell)
 ...
 call getneigh1(node,xpos,xsizei,rcuti,ndim,listneigh,nneigh,xyzh,xyzcache,ixyzcachesize,.false.)
