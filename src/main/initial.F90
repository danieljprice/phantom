 ...
 do j = 1,nptmass
    ...
    if (iexist) then
       ...
       if (dumpfile(i-2:i) == 'tmp') then
          if (j==1) file1D = dumpfile(1:i-9) // '1D.dat'
          if (j==2) file1D = dumpfile(1:i-9) // '1D' // char(48+j) // '.dat'
       else
          if (j==1) file1D = dumpfile(1:i-5) // '1D.dat'
          if (j==2) file1D = dumpfile(1:i-5) // '1D' // char(48+j) // '.dat'
       endif
