...
subroutine inject_particles(time,dtlast,xyzh,vxyzu,xyzmh_ptmass,vxyz_ptmass,&
 ...
 real :: dtrel_min, dr_min ! for diagnostic
 dtrel_min = 1e30
 dr_min    = 1e30
 ...
 if (xyzmh_ptmass(ihsoft,1) > 0.) then
   ...
   do i=1,npart
     ...
     if (norm2(rel_r) < star_r) then
        ...
        rmag  = norm2(rel_r)
        if (rmag < star_r) rel_r = rel_r * (star_r/rmag) * 1.001 ! if still inside (or numerically on surface), push slightly outside

        ! OLD-style reposition: always place outside by 1%
        !rel_r = dir_r * star_r * 1.01

        ! diagnostic
        dtrel_min = min(dtrel_min, dt - thit)      ! smallest remaining time
        dr_min    = min(dr_min,    rmag - star_r)  ! smallest surface clearance
   ...
   enddo
   print*,' min(dt-thit)=',dtrel_min,' min(r-R)=',dr_min
