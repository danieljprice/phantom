...
!print*,i,T/Tfermi(ne)
 ...
 subroutine bisection_p(d,p,T,mue,mui,tol)
  ...
  real(8):: t1,t2,p1,p2,newt,newp,fac=0.5d0,eta=0d0
  integer:: i

  call get_p_from_dT(d,T,mue,mui,p1,eta=eta);p1=p1-p
  ...
  if(p1 > 0)then
   ...
   do
    t1 = fac*t1
    p2 = p1
    call get_p_from_dT(d,t1,mue,mui,p1,eta=eta);p1=p1-p
    if(p1<0)exit
   end do
   t2 = t1/fac
  else
   ...
   do
    t2 = t2/fac
    p1 = p2
    call get_p_from_dT(d,t2,mue,mui,p2,eta=eta);p2=p2-p
    if(p2>0)exit
   end do
   t1 = t2*fac
  end if
  ...
  do i = 1, 1000
   newt = 0.5d0*(t1+t2)
   call get_p_from_dT(d,newt,mue,mui,newp,eta=eta);newp=newp-p
   if(p1*newp>0d0)then
    t1 = newt
    p1 = newp
   else
 ...
 subroutine etaT_from_de(d,e,mue,mui,eta,T)
  ...
  real(8),intent(inout):: eta,T
  real(8):: f,dfdt,corr,logT
  integer:: i,imax=1000
  real(8):: u,dudt,ne,  Tdot,dt
...
! Get good initial guess if guess is not provided
  ...
  do i = 1, imax
   ...
   call get_u_from_dT(d,T,mue,mui,u,eta)
 ...
 subroutine bisection_u(d,u,T,mue,mui,tol)
  ...
  real(8):: t1,t2,u1,u2,newt,newu

  call get_u_from_dT(d,T,mue,mui,u1);u1=u1-u
  ...
  do
   ...
   if(abs(newu/u)<tol.or.t2/t1-1d0<tol)exit
  end do
 ...
 subroutine get_pT_from_de_ryo(d,e,mue,mui,p,T,eta)
  ...
  if(eta1<etamax)then
   theta = fac_theta*T
   call get_pele(eta1,theta,pele,nele=nele)
   p = pele + pion(d,T,mui) + prad(T)
   if(present(eta))eta=eta1
  else
   p = Pdeg(nele)
   if(present(eta))eta=etamax
  end if
 ...
 subroutine get_eT_from_dp_ryo(d,p,mue,mui,e,T,eta)
  ...
  if(eta1<etamax)then
   theta = fac_theta*T
   call get_eele(eta1,theta,eele,nele=nele)
...
! Limit eta value to <etalim and use cold formula instead
 ...
 function Efermi(ne) result(Ef)
  ...
  real(8),intent(in):: ne
  real(8):: Ef, const, f, dfde, corr
  Ef = hbar*hbar/(2d0*m_e)*(3d0*pi*pi*ne)**(2d0/3d0)
  const = (pfermi(ne)*clight)**2
  do
   f = Ef**2+2d0*Ef*m_e*clight*clight-const
   dfde = 2d0*(Ef+m_e*clight*clight)
   corr = -f/dfde
   Ef = Ef + corr
   if(abs(f/const)<err)exit
  end do
 end function Efermi
