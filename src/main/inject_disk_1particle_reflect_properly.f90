...
subroutine inject_particles(time,dtlast,xyzh,vxyzu,xyzmh_ptmass,vxyz_ptmass,&
 ...
 if (xyzmh_ptmass(ihsoft,1) > 0.) then
   ...
   rel_r = xyzh(1:3,1) - star_xyz
   if (min_d > norm2(rel_r)) min_d = norm2(rel_r)
   if (norm2(rel_r) < star_r) then
      nbounce = nbounce + 1

      dt  = dtlast
      rel_v = vxyzu(1:3,1) - star_v   ! relative velocity (star frame)
      ...
      ! back to lab frame
      xyzh(1:3,1)  = star_xyz + rel_r
      vxyzu(1:3,1) = rel_v   + star_v
    endif

   print *,'Did bounce:',nbounce,'min_d=',min_d, 'eps=',eps, 'dt=',dt
 endif
