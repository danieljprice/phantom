...
! Read from input file
 ...
 integer, public :: idir=-1 ! Wind direction (+1 = outward, -1 = inward)
...
subroutine inject_particles(time_u,dtlast_u,xyzh,vxyzu,xyzmh_ptmass,vxyz_ptmass,npart,npartoftype)
 ...
 ! print*,inner_sphere+ihandled_spheres,outer_sphere,-idir
 ! stop 'asdasd'
...
subroutine drv_dt(rv, drv, GM)
 ...
 dr_dt = v
 r2 = r*r
 T = wind_temperature * (wind_injection_radius**2 * wind_velocity / (r2 * abs(v)))**(wind_gamma-1.)
...
subroutine compute_sphere_properties(local_time, r, v, u, rho, e)
 ...
 dt = idir*local_time / N
 rv(1) = wind_injection_radius
 rv(2) = idir*wind_velocity
 if (nptmass == 0) then
    GM = 0.
 else
 ...
 r = rv(1)
 v = rv(2)
 T = wind_temperature * (wind_injection_radius**2 * wind_velocity / (r**2 * abs(v)))**(wind_gamma-1.)
 u = T * u_to_temperature_ratio
 rho = wind_mass_rate / (4.*pi*r**2*abs(v))
