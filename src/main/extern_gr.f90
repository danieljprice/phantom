...
subroutine get_grforce(xyzhi,metrici,metricderivsi,veli,densi,ui,pi,fexti,dtf)
 ...
 if (ierr > 0) then
    ...
    if (imetric==imet_binarybh) then
       print*,' distance from bh1 = ',sqrt(dot_product(xyzhi(1:3)-bh_trajectory(1:3),xyzhi(1:3)-bh_trajectory(1:3)))
       print*,' distance from bh2 = ',sqrt(dot_product(xyzhi(1:3)-bh_trajectory(4:6),xyzhi(1:3)-bh_trajectory(4:6)))
    endif
