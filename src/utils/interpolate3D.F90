...
subroutine interpolate3D(xyzh,weight,npart, &
                             xmin,datsmooth,nnodes,dxgrid,normalise,dat,ngrid)
...
!  xminpix(:) = xmin(:) - 0.5*dxcell(:)
 ...
 print*, "position of i cell 4 is: ", 4*dxcell(1) + xmin(1) 
 ...
 !$omp shared(npart,xyzh,dat,datsmooth,datnorm)  &
 !$omp firstprivate(const,weight)                        &
 !$omp firstprivate(xmin,imesh,nnodes,level)               &
 !$omp firstprivate(npixx,npixy,npixz,dxmax,dxcell,normalise)    &
 !$omp private(i,j,hi,hi1,hi21,radkern,termnorm,term)    &
