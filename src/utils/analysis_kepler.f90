...
subroutine phantom_to_kepler_arrays(xyzh,vxyzu,pmass,npart,time,density,rad_grid,mass_enclosed,bin_mass,&
 ...
 integer ::particle_bound_bh,last_particle_with_neg_e,energy_verified_no,index_val,i_next
...
!Determining centre of star as max density particle 
 ...
 composition_sum(:) = 0.
 composition_i(:)   = 0.
 open(10,file="particleOmega.info")
 write(10,*) "[pos]"," ","[omega]"
 open(11,file="binOmega.info")
 write(11,*) "[pos]"," ","[omega]"
 open(101,file="radius_of_bins.info")
 write(101,*) "[ibin]"," ","[rad_inner]"," ","[rad_outer]"," ","[Position rad next]"," ","[particles in bin]"
 ...
 !   print*,vel,"vel of particle",j,"j"
    ...
    write(10,*)pos_mag,omega_particle 
    !Moment of inertia
    ...
    if (count_particles==number_per_bin .or. j==energy_verified_no) then 
        ...
        write(11,*)pos_mag,omega_bin
        composition_kepler(:,ibin) = composition_sum(:)/count_particles
        !print*,composition_kepler(:,ibin)
       write(101,*) ibin,rad_inner,rad_outer,pos_mag_next,count_particles
       ! print*,rad_inner,"innerradius",rad_outer,"outerradius",ibin,"ibin",rad_grid(ibin),"rad grid ibin",udist
        count_particles = 0
        density_sum     = 0.
        temperature_sum = 0.
        rad_mom_sum     = 0.
...
subroutine particle_pos_and_vel_wrt_centre(xpos,vpos,xyzh,vxyzu,pos,vel,i,pos_mag,vel_mag)
 ...
 !+
 !----------------------------------------------------------------
subroutine particles_bound_to_star(xpos,vpos,xyzh,vxyzu,pmass,npart,iorder,energy_verified_no,last_particle_with_neg_e,array_particle_j)
 ...
 integer,intent(in)               :: npart,iorder(:)
 ...
 index_val = 1
 dummy_size = 1e8
 allocate(index_particle_star(dummy_size))
 do j = 1, npart
