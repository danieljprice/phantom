...
module analysis
  ...
  ! :References: None
  !
  ! :Author: Megha Sharma (Supervisors- Daniel Price and Alexander Heger)
  !
    ...
    do j = 1, npart
     ...
     if (no_in_bin >= number_particle .and. ibin /= ngrid)  then
       ...
       if (j < npart) then
         ...
         !make the bin properties.
         print*, ibin, 'ibin',no_in_bin,'no_in_bin for last bin'
         !for last bin, we consider the radius of the last particle in the current bin.
