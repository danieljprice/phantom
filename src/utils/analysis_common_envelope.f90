...
subroutine tau_profile(time,num,npart,particlemass,xyzh,vxyzu)
 use part, only:eos_vars,itemp,ikappa
 use eos,  only:equationofstate,eos_is_non_ideal
 use radiation_utils, only:Tgas_from_ugas
 ...
 real                   :: maxloga,minloga,kappa,kappat,kappar,tempi
 ...
 do i=1,npart
    ...
    if (do_radiation) then
       kappa_part(i) = radprop(ikappa,i)*unit_opacity
    else
       if (eos_is_non_ideal(ieos)) then
          tempi = eos_vars(itemp,i)
       elseif (do_radiation) then
          tempi = Tgas_from_ugas(vxyzu(4,i),gamma,gmw)
       else
          call equationofstate(ieos,ponrhoi,spsoundi,rho_part(i),xyzh(1,i),xyzh(2,i),xyzh(3,i),tempi,vxyzu(4,i))
       endif
       call get_eos_kappa_mesa(rho_part(i)*unit_density,tempi,kappa,kappat,kappar)
       kappa_part(i) = kappa
    endif
 enddo
