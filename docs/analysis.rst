...
Phantomanalysis is a simple wrapper that reads all of the dump files on the command line in sequence and calls the analysis routine specified in the analysis_blah.f90 module. You can also select the module on the command line using, for example
...
- actually performing some analysis (e.g. `analysis_disc.f90 <https://github.com/danieljprice/phantom/blob/master/src/utils/analysis_disc.f90>` which bins particles into rings for comparison with 1D alpha-disc evolution calculations).
